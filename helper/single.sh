...
if [ "$checkproof" == "yes" ] && ([ "$status_withPL" == "ok" ] || [ "$checkpreviousstep" == "no" ]); then
  ...
  $loc_bin/runlim -r $TIMEOUT_VERIPB -s $MEMOUT_VERIPB -o $loc_outputs/$experiment_name/${filename}_verification.txt $loc_bin/veripb -s -c --wcnf $instance $loc_proofs/$experiment_name/${filename}_proof.pbp > $loc_outputs/$experiment_name/${filename}_veripb_output.txt 2>&1
  ...
  if grep -q "VERIFIED BOUNDS" $loc_outputs/$experiment_name/${filename}_veripb_output.txt; then
